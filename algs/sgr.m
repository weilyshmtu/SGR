function [S, C, F, obj_val, acc_array1, acc_array2] = sgr(K, alpha, beta, opts)
% \min ||x_i- x_j||S_ij + alpha*||s_i - s_j||C_ij + beta*||S - C||_F^2
% s.t. Se = e, S = S', S>=0, diag(S) = 0；
%      Ce = e, C = C', C>=0, diag(C) = 0 , rank(L_C) = n-c;
...
while iter < max_iter
    ...
    % update S
    [S, S_res] = solve_S(K, S, C, alpha, beta, opts);
  
    % update C
    [C, C_res] = solve_C(S, C, FFt, alpha, beta, lambda, opts);

    % update F
    [F, lambda, bflag] = solve_F(C, F, lambda, opts);
    FFt = F*F';
...
function [S, S_residual] = solve_S(K, S_pre, C, alpha, beta, opts)
    n = opts.n_samples;
    Y = zeros(n, n);
    N = Y;
    S = Y;
...
function [C, C_residual] = solve_C(S, C_pre, FFt, alpha, beta, lambda, opts)
    n = opts.n_samples;
    ...
    while iter < max_iter
        ...
        for i = 1:n
            ...
            Bi = alpha*(S(index,index)*St(index,i)) + 2*beta*S(index,i) + lambda*FFt(index, i);
            Vi = V(index,i);
            Yi = Y(index,i);
            m =  Vi- (Yi + Ai*Vi - Bi)/mu;
...
function  [F, lambda, bflag] = solve_F(C, F, lambda,  opts)
    bflag = false;
    c = opts.n_clusters;
    zr = 1e-10;
    F_old = F;
    ...
    C = 0.5*C;
    L_C = diag(sum(C,2)) - C;
    [F, ev] = eigs(L_C, c+1, "smallestreal");
    F = real(F(:, 1:c));
